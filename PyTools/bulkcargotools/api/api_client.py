...
username = 'admin@24vision.com'
password = 'PSbi4macom'

def query_api(endpoint, method, parameters={}, inputdata={}, format="json", limit="200"):
...
def query_api_get_classificationtraining(limit : object = "200") -> object:
    return requests.get(basedomain +"/api"+ "/emails/classificationtraining/"+limit,auth=HTTPBasicAuth(username, password)
							)

def query_api_get_unextractedcargo(limit : object = "200") -> object:
    return requests.get(basedomain +"/api"+ "/emails/unextracted-cargo/"+limit, auth=HTTPBasicAuth(username, password)
							)

def query_api_get_unextractedship(limit : object = "200") -> object:
    return requests.get(basedomain +"/api"+ "/emails/unextracted-ship/"+limit,auth=HTTPBasicAuth(username, password)
							)

def query_api_get_unextractedorder(limit : object = "200") -> object:
    return requests.get(basedomain +"/api"+ "/emails/unextracted-order/"+limit,auth=HTTPBasicAuth(username, password)
                                                        )
...
def query_api_get_unclassified_emails(limit : object = "200") -> object:
    return requests.get(basedomain +"/api"+ "/emails/unclassified/"+limit,
                            auth=HTTPBasicAuth(username, password)
                                                        )
...
def query_api_store_distance(data):
	return requests.post(basedomain +"/api"+ "/distances", data,
                            auth=HTTPBasicAuth(username, password)
                                                        )

def query_api_set_cleaned_cargo_offer(id):
        return requests.put(basedomain +"/api"+ "/setCleaned/"+id,
                            auth=HTTPBasicAuth(username, password)
                                                        )
