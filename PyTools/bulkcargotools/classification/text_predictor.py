...
#For testing we deleted the filter : {"filter": "unclassified"}
#emails = requests.get("http://developer.cargoinship.com/api/emails/unclassified/5000")
emails =    api_client.query_api_get_unclassified_emails(str(5000)) #Emails from API
...
print("no of fetched mails in text_predictor: "+ str(counter))
print("length of mailCorpus in text predictor: "+str(len(mailCorpus)))
...
for mID, pred , proba  in zip(mailID, predictions , output_proba):
    ...
    print("post data to db (text_predictor)")
    #res4 = requests.put("http://developer.cargoinship.com/api/emails/"+str(mID), data=classif_data) #Send the response to the server.
    res4 = api_client.query_api("emails/"+str(mID),"put",{}, data,"json", str(5000))
