<?php
/*
* Generic endpoint that is used to fulfill requests to tables for which no specific endpoint has been defined. Offers basic functionality for all tables.
...
class EndpointGeneric extends Endpoint {
	...
	function __construct($type, $typeid = null, $subtype = null, $subtypeid = null) {
		$this->db = $GLOBALS["db"];
		$this->type = $GLOBALS["INPUT_PARAMS"]["type"];
		$this->typeid = $typeid;
		
		if (isset($this->tableassignment[strtolower($this->type)])) {
			$this->table = $this->tableassignment[strtolower($this->type)];
			...
			throw new Exception("Generic endpoint not available for type " . $this->type); 
		}
    }
