<?php
/* 
Endpoint child class that handles requests for the Email Endpoint.
*/
class EndpointEmails extends Endpoint {
	...
	function get($body = null) {
		...
		//Default case
		$sql = "SELECT emailID, subject, body, sender, receiver, cc, date, classification_manual, classification_automated, classification_automated_certainty FROM email LIMIT " . $GLOBALS["INPUT_PARAMS"]["limit"];
		$headers = array("emailID", "subject", "body", "sender", "receiver", "cc", "date", "classification_manual", "classification_automated", "classification_automated_certainty");
		...
		//Filter used by classification scripts to get unclassified emails
		...
		//Filter used by classification training script to retrieve training data.
		...
		//3 Filters used by the 3 extraction scripts respectively to get unextracted emails. 
		if ($GLOBALS["INPUT_PARAMS"]["filter"] == "unextracted-cargo") { 
			$sql = "SELECT * FROM email WHERE emailID NOT IN (SELECT email.emailID FROM email INNER JOIN cargo_offer_extracted on email.emailID = cargo_offer_extracted.emailID GROUP BY email.emailID) AND classification_automated='Cargo' ORDER BY _created_on DESC LIMIT " . $GLOBALS["INPUT_PARAMS"]["limit"];
		}
		if ($GLOBALS["INPUT_PARAMS"]["filter"] == "unextracted-ship") {
			$sql = "SELECT * FROM email WHERE emailID NOT IN (SELECT email.emailID FROM email INNER JOIN ship_offer_extracted on email.emailID = ship_offer_extracted.emailID GROUP BY email.emailID) AND classification_automated='Ship' ORDER BY _created_on DESC LIMIT " . $GLOBALS["INPUT_PARAMS"]["limit"];
		}
		if ($GLOBALS["INPUT_PARAMS"]["filter"] == "unextracted-order") {
			$sql = "SELECT * FROM email WHERE emailID NOT IN (SELECT email.emailID FROM email INNER JOIN ship_order_extracted on email.emailID = ship_order_extracted.emailID GROUP BY email.emailID) AND classification_automated='Order' ORDER BY _created_on DESC LIMIT " . $GLOBALS["INPUT_PARAMS"]["limit"];
		}
		...
		//2 Kibana-specific filters.
		if ($GLOBALS["INPUT_PARAMS"]["filter"] == "kibana-unextracted") {
			$sql = "SELECT emailID, subject, body, sender, receiver, cc, date, classification_manual, classification_automated, classification_automated_certainty FROM email WHERE kibana_extracted = 0 OR kibana_extracted IS NULL LIMIT " . $GLOBALS["INPUT_PARAMS"]["limit"];
		}
		if ($GLOBALS["INPUT_PARAMS"]["filter"] == "kibana-extracted") {
			$sql = "SELECT emailID, subject, body, sender, receiver, cc, date, classification_manual, classification_automated, classification_automated_certainty FROM email WHERE kibana_extracted = 1 LIMIT " . $GLOBALS["INPUT_PARAMS"]["limit"];
		}
