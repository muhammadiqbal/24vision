...
if __name__ == 'model':
    ...
    from extraction.sequence_tagging.data_utils import minibatches, pad_sequences, get_chunks
    from extraction.sequence_tagging.general_utils import Progbar, print_sentence
...
class NERModel(object):
    ...
    def add_word_embeddings_op(self):
        ...
        with tf.variable_scope("chars"):
            if self.config.chars:
                ...
                # bi lstm on chars
                lstm_cell_fw = tf.contrib.rnn.LSTMCell(self.config.char_hidden_size,
                                                       state_is_tuple=True)
                lstm_cell_bw = tf.contrib.rnn.LSTMCell(self.config.char_hidden_size,
                                                       state_is_tuple=True)
                _, ((_, output_fw), (_, output_bw)) = tf.nn.bidirectional_dynamic_rnn(lstm_cell_fw,
                                                                                      lstm_cell_bw, char_embeddings, sequence_length=word_lengths,
                                                                                      dtype=tf.float32)
    ...
    def add_logits_op(self):
        ...
        with tf.variable_scope("bi-lstm"):
            lstm_cell_fw = tf.contrib.rnn.LSTMCell(self.config.hidden_size)
            lstm_cell_bw = tf.contrib.rnn.LSTMCell(self.config.hidden_size)
            (output_fw, output_bw), _ = tf.nn.bidirectional_dynamic_rnn(lstm_cell_fw,
                                                                        lstm_cell_bw, self.word_embeddings, sequence_length=self.sequence_lengths,
                                                                        dtype=tf.float32)
            output = tf.concat([output_fw, output_bw], axis=-1)
    ...
    def add_summary(self, sess):
        ...
        self.file_writer = tf.summary.FileWriter(
            self.config.output_path, sess.graph)
    ...
    def run_epoch(self, sess, train, dev, tags, epoch):
        ...
        for i, (words, labels) in enumerate(minibatches(train, self.config.batch_size)):
            fd, _ = self.get_feed_dict(
                words, labels, self.config.lr, self.config.dropout)
        ...
        acc, p, r, f1 = self.run_evaluate(sess, dev, tags)
        self.logger.info(
            " | dev acc: {:04.2f}% | precision: {:04.2f}% | recall: {:04.2f}% | f1: {:04.2f}%".format(acc*100, p*100, r*100, f1*100))
        return acc, p, r, f1
    ...
    def run_evaluate(self, sess, test, tags):
        ...
        for words, labels in minibatches(test, self.config.batch_size):
            ...
            for lab, lab_pred, length in zip(labels, labels_pred, sequence_lengths):
                ...
                accs += [a_b[0] == a_b[1] for a_b in zip(lab, lab_pred)]
    ...
    def train(self, train, dev, tags):
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(self.config.nepochs):
                ...
                acc, _, _, f1 = self.run_epoch(sess, train, dev, tags, epoch)
    ...
    def evaluate(self, test, tags):
        ...
        with tf.Session() as sess:
            ...
            acc, p, r, f1 = self.run_evaluate(sess, test, tags)
            self.logger.info(
            " | dev acc: {:04.2f}% | precision: {:04.2f}% | recall: {:04.2f}% | f1: {:04.2f}%".format(acc*100, p*100, r*100, f1*100))

    def predict_text(self, text, tags, processing_word):
        idx_to_tag = {idx: tag for tag, idx in tags.items()}
        saver = tf.train.Saver()
        with tf.Session() as sess:
            saver.restore(sess, self.config.model_output)
            text_list = list(filter(None, text.split("\n")))
            text_list = filter(lambda line: line.strip(), text_list)
            ...
            for line in text_list:
                ...
                prediction_list.extend(list(zip(words_raw, preds)))
            return prediction_list

    def interactive_shell_one_line(self, tags, processing_word):
        idx_to_tag = {idx: tag for tag, idx in tags.items()}
        saver = tf.train.Saver()
        with tf.Session() as sess:
            saver.restore(sess, self.config.model_output)
    ...
    def interactive_shell_multiline(self, tags, processing_word):
        idx_to_tag = {idx: tag for tag, idx in tags.items()}
        saver = tf.train.Saver()
        with tf.Session() as sess:
            saver.restore(sess, self.config.model_output)
            ...
            while True:
                try:
                    self.logger.info("This is an interactive mode. Enter an email, then type \"PREDICT!\" and hit enter:")
                    text = ""
                    while True:
                        line = input()
