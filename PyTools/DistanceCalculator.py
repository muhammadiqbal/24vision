...
import sys
import requests
import bulkcargotools.api.api_client as api_client
from osgeo import gdal

#Transforming a raster map to array datatype.
raster = gdal.Open('/var/www/24vision/PyTools/map.tif')
...
#ports = pd.read_csv('/Project Seminar/Distance Calculator/port.csv')
...
#A test to store object using pickle
#file = open('/Distance Calculator_Python/test.obj', 'wb')
#pickle.dump(mapArray, file)

#filehandler = open('/Distance Calculator_Python/test.obj', 'rb')
#mapArray2 = pickle.load(filehandler)
...
startCoord = (float(sys.argv[3]), float(sys.argv[4]))
stopCoord = (float(sys.argv[5]), float(sys.argv[6]))

start_port_ID = sys.argv[1]
end_port_ID = sys.argv[2]
distance = distanceCalculator(startCoord, stopCoord)
data = {"start_port": start_port_ID, "end_port": end_port_ID, "distance": distance}

result = api_client.query_api_store_distance(data)
if result.status_code != 200:
	print ("Could not save distance into DB. ERROR:" + str(result.status_code))
else:
	print("Distance saved into DB. HTTP code:" + str(result.status_code))
# requests.post("http://developer.cargoinship.com/api/distances/"+str(mID), )

#print(distanceCalculator(startCoord, stopCoord))
