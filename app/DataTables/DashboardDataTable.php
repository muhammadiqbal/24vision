...
class DashboardDataTable extends DataTable
{
    ...
    public function query()
    {
        $cargos = Cargo::leftjoin('cargo_status', 'cargo_status.id','cargo_status.id')
                        ...
                        ->where(DB::raw('quantity * stowage_factor AS size'),'<=',  ($this->ship->max_holds_capacity - $this->occupied_size))
                        ->where(DB::raw('quantity *'.$this->ship->ballast_draft),'<=', ($this->ship->max_laden_draft-($this->ship->ballast_draft * $this->occupied_tonage)))
