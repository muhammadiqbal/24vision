...
class VoyageController extends Controller
{
    ...
    function getVoyage(Ship $ship, Cargo $cargo, Port $port_ship,$date, Calculator $calculator){
		...
		$fuel_array = $calculator->createFuelArray($cargo, $ship, $date, $travel_time_to_start, $travel_time_cargo, $port_time_load, $port_time_disch) ;
		$fuel_total_costs =  $calculator->calculateTotalFuelCosts(($fuel_array);
		...
		$non_hire_costs =  $calculator->calculateNonHireCosts($fuel_total_costs, $port_fee_load, $port_fee_disch);
		
		$fuel_array_bdi = $calculator->createFuelArray($cargo, $ship_bdi, $date, $travel_time_to_start_bdi, $travel_time_cargo_bdi, $port_time_load, $port_time_disch) ;
		$fuel_total_costs_bdi =  $calculator->calculateTotalFuelCosts(($fuel_array_bdi);
		...
		$non_hire_costs_bdi =  $calculator->calculateNonHireCosts($fuel_total_costs_bdi, $port_fee_load_bdi, $port_fee_disch_bdi);
		...
		->with('fuel_array',$fuel_array)
		->with('fuel_total_costs',$fuel_total_costs)
